...
case "$code" in
  ...
  *)
    ...
    say "  Where: claude.ai console -> Organization settings -> Capabilities -> Code execution section."
    say "  Keep the 'Domain allowlist' dropdown on 'Package managers only', and under"
    say "  'Additional allowed domains' add these four entries (press Add after each):"
    say "    youtube.com"
    say "    *.youtube.com"
    say "    *.googlevideo.com"
    say "    *.ytimg.com"
    say "  Known issue: additional domains are not reliably enforced right now"
    say "  (github.com/anthropics/claude-code issues #51400, #30112, #38984)."
    say "  If adding them doesn't take effect, switch the Domain allowlist dropdown to the"
    say "  widest option ('All domains') for this workspace until those issues are fixed."
    say "  The setting is read when a session starts — start a NEW Cowork session after changing it."
    exit 30
    ;;
esac
exit 0
